...
const FaqComponentHome = memo(({ className = "", faqs }) => {
        ...
        <div className="flex flex-row items-center justify-center px-0 pb-0 box-border max-w-full mq750:box-border mq750:min-w-full">
          <div className="self-stretch flex flex-col items-center justify-center gap-10 max-w-full mq750:gap-5">
            <div className="self-stretch flex flex-col items-start justify-center py-0 mq480:items-center mq480:px-5 box-border max-w-full">
              <h1 className="m-0 relative text-inherit leading-[120%] font-medium font-[inherit] text-center max-w-full mq480:text-xl mq750:text-13xl mq750:leading-[38px] mq480:text-5xl mq480:leading-[29px]">
                Frequently Asked Questions
              </h1>
            </div>
            <div className="self-stretch flex flex-col items-center justify-start text-xl" data-acc-group>
              {faqs.map((faq, index) => (
                <div
                  key={index}
                  className={`w-full max-w-[612px] border-gray-800 border-b-[1px] border-solid box-border flex flex-col items-start justify-start [transition-property:all] ease-[cubic-bezier(0.4,_0,_0.2,_1)] duration-[150ms] ${openIndex === index ? "open" : ""}`}
                  data-acc-item
                >
                  <div
                  ...
                  >
                    ...
                    <Image
                      className={`h-8 w-8 relative overflow-hidden shrink-0 transform transition-transform duration-150 ${openIndex === index ? "rotate-90" : ""}`}
                      width={32}
                      height={32}
                      alt="Toggle FAQ"
                      src="/plusIcon.png"
                  ...
                  <div
                    className={`w-full grid ${openIndex === index ? "grid-rows-[1fr]" : "grid-rows-[0fr]"} [transition-property:all] ease-[cubic-bezier(0.4,_0,_0.2,_1)] duration-[150ms] text-base`}
                    data-acc-content
                  >
